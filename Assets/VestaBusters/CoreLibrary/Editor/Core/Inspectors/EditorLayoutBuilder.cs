...
namespace VestaBusters.CoreLibrary.Editor
{
    public class EditorLayoutBuilder
    {
            ...
            params string[] ignoreProperties)
        {
            try
            {
                ...
                while (currentProperty.NextVisible(enterChildren: firstTime))
                {
                    ...
                    if ((ignoreProperties != null) && System.Array.Exists(ignoreProperties,
                            (item) => string.Equals(item, currentProperty.name)))
                    {
                        continue;
                    }
                    ...
                    if (prefix != null)
                    {
                        EditorGUILayout.PropertyField(currentProperty,
                            new GUIContent($"{prefix} {currentProperty.displayName}", currentProperty.tooltip), true);
                    }
                    else
                    {
        ...
        private void DrawTabBar()
        {
            if (m_tabs.Length > 1)
            {
                m_tabBarScrollPosition =
                    GUILayout.BeginScrollView(m_tabBarScrollPosition, m_backgroundStyle, GUILayout.Height(30f));
            ...
            if ((m_drawTabViewCallback == null) || !m_drawTabViewCallback(m_selectedTab))
            {
                for (int iter = 0; iter < m_selectedTabSections.Length; iter++)
                {
                    ...
                    DrawSection(section: current,
                        showDetails: (current.DrawStyle == EditorSectionDrawStyle.Expand) &&
                                     (current == m_focusSection));
                }
            }
        }
