...
namespace VestaBusters.CoreLibrary.Editor
{
    public static class ScriptingDefinesManager
    {
        ...
        private static void AddDefineToCollection(Dictionary<BuildTargetGroup, List<string>> definesCollection,
            string define, BuildTargetGroup[] targetGroups)
        {
                ...
                foreach (var targetGroup in s_supportedTargetGroups)
                {
                    ...
                    if (isModified)
                    {
                        PlayerSettings.SetScriptingDefineSymbolsForGroup(targetGroup,
                            string.Join(";", updatedDefines.ToArray()));
                    }
                }
            }
            finally
