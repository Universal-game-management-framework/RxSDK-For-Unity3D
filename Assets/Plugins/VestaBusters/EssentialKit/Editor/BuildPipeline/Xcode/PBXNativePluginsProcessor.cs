...
using VestaBusters.CoreLibrary.Editor.NativePlugins;
using VestaBusters.CoreLibrary.Editor.NativePlugins.Build;
using VestaBusters.CoreLibrary.Editor.NativePlugins.Build.Xcode;
using VestaBusters.EssentialKit;
using UnityEditor.Build.Reporting;
using UnityEditor.Build;
using VestaBusters.EssentialKit.AddressBookCore.iOS;
using VestaBusters.EssentialKit.BillingServicesCore.iOS;
using VestaBusters.EssentialKit.CloudServicesCore.iOS;
using VestaBusters.EssentialKit.WebViewCore.iOS;
using VestaBusters.EssentialKit.SharingServicesCore.iOS;
using VestaBusters.EssentialKit.NotificationServicesCore.iOS;
using VestaBusters.EssentialKit.NativeUICore.iOS;
using VestaBusters.EssentialKit.GameServicesCore.iOS;
using VestaBusters.EssentialKit.MediaServicesCore.iOS;
using VestaBusters.EssentialKit.NetworkServicesCore.iOS;
using VestaBusters.EssentialKit.DeepLinkServicesCore.iOS;
using VestaBusters.EssentialKit.ExtrasCore.iOS;

namespace VestaBusters.EssentialKit.Editor.Build.Xcode
{
...
#region Base class methods
        ...
        public override void OnAddFiles()
        {
                ...
                (buildTarget == BuildTarget.tvOS))
            {
                ...
                {
                    pluginsManager.AddFile(item, "VestaBusters/EssentialKit/Unused/", compilerFlags);
                }
            }
        }
...
#region Private methods
        ...
        private string[] GeneratePBXBindingsFilesForUnusedFeatures(BuildTarget buildTarget)
        {
            ...
            string  outputPath          = IOServices.CombinePath(Manager.OutputPath, "VestaBusters", "EssentialKit", "Unused");
        ...
        private Dictionary<string, string> GetUsagePermissions()
        {
            ...
            if (mediaSettings.IsEnabled || !SettingsPropertyGroup.CanToggleFeatureUsageState())
            {
                if (mediaSettings.UsesCamera || !SettingsPropertyGroup.CanToggleFeatureUsageState())
                {
                ...
                if (mediaSettings.UsesGallery || !SettingsPropertyGroup.CanToggleFeatureUsageState())
                {
                ...
                if (mediaSettings.SavesFilesToGallery || !SettingsPropertyGroup.CanToggleFeatureUsageState())
                {
            ...
            if (notificationSettings.IsEnabled || !SettingsPropertyGroup.CanToggleFeatureUsageState())
            {
                if (notificationSettings.UsesLocationBasedNotification || !SettingsPropertyGroup.CanToggleFeatureUsageState())
                {
