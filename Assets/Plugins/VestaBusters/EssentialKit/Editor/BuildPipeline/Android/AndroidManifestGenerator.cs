...
#region Static fields
        ...
        private static string s_androidLibraryRootPackageName = "com.vestabusters.essentialkit";
...
#region Private methods
        ...
        private static void AddActivities(Application application, EssentialKitSettings settings)
        {
            ...
            /*if (settings.WebViewSettings.IsEnabled)
            {
                ...
                activity.AddAttribute("android:name", s_androidLibraryRootPackageName + ".webview.WebViewActivity");
            ...
            if (settings.DeepLinkServicesSettings.IsEnabled)
            {
                ...
                if(universalLinks.Length > 0 || urlSchemes.Length > 0)
                {
                    ...
                    activity.AddAttribute("android:label", UnityEditor.PlayerSettings.productName);//As we don't have app name available in manifest directly, we are creating this entry here. Else could have included directly in the aar's AndroindManifest file.
            ...
            {
                activity = new Activity(); 
                activity.AddAttribute("android:name", s_androidLibraryRootPackageName + ".notificationservices.NotificationLauncher");
                activity.AddAttribute("android:theme", "@style/Theme.Transparent");
                activity.AddAttribute("android:exported", "true");
                application.Add(activity);
            }*/
        ...
        private static void AddProviders(Application application, EssentialKitSettings settings)
        {
            ...
            provider.AddAttribute("android:name", "com.vestabusters.essentialkit.common.FileProviderExtended");
        ...
        private static void AddServices(Application application, EssentialKitSettings settings)
        {
            /*if(settings.NotificationServicesSettings.IsEnabled && settings.NotificationServicesSettings.PushNotificationServiceType == PushNotificationServiceType.Custom)
            {
                ...
                service.AddAttribute("android:name", s_androidLibraryRootPackageName + ".notificationservices.provider.fcm.FCMMessagingService");
        ...
        private static void AddReceivers(Application application, EssentialKitSettings settings)
        {
            ...
            if(settings.NotificationServicesSettings.IsEnabled)
            {
                ...
                receiver.AddAttribute("android:name", s_androidLibraryRootPackageName + ".notificationservices.AlarmBroadcastReceiver");
                application.Add(receiver);

                receiver = new Receiver();
                receiver.AddAttribute("android:name", s_androidLibraryRootPackageName + ".notificationservices.BootCompleteBroadcastReceiver");
        ...
        private static void AddPermissions(Manifest manifest, EssentialKitSettings settings)
        {
            ...
            if (settings.NotificationServicesSettings.IsEnabled)
            {
                ...
                if(!settings.NotificationServicesSettings.AndroidProperties.AllowInExactTimeScheduling)
                {
                    permission = new Permission();
                ...
                /*permission = new Permission();
                permission.AddAttribute("android:name", "android.permission.POST_NOTIFICATIONS");
                manifest.Add(permission); */
            }
